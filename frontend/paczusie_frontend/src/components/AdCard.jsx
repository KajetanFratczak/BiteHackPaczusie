...
const AdCard = ({ id, title, description, images, categories }) => {
      ...
      <div className="p-[15px] text-left flex flex-col flex-grow">
        ...
        <div className="flex flex-wrap gap-2 mt-auto">
          {categories && categories.map((cat, index) => (
            <span
              key={index}
              className="bg-gray-100 text-gray-500 text-[12px] px-2 py-1 rounded font-bold uppercase tracking-wider border border-gray-200 mt-3"
            >
              {cat}
            </span>
          ))}
